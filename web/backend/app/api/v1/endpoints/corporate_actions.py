...
from app.models.corporate_action import CorporateAction, CorporateActionStatus, IndexCorporateActionLog
...
async def get_corporate_action(
    ...
    result = await db.execute(
        select(CorporateAction).where(CorporateAction.id == action_id)
    )
...
async def apply_corporate_action(
    ...
    if action.action_type in ["stock_split", "reverse_split"]:
        if action.ratio:
            ...
            action_taken = f"Applied {action.ratio}:1 split. Shares: {old_shares:.2f} -> {new_shares:.2f}"
    
    elif action.action_type == "cash_dividend":
        # Record dividend for total return calculation
        action_taken = f"Recorded cash dividend: {action.dividend_currency or 'USD'} {action.dividend_amount}"
        ...
        if action.acquirer_ticker:
            ...
            acquirer_exists = any(c.ticker == action.acquirer_ticker and c.is_active for c in index.components)
            
            if acquirer_exists:
                # Merge into existing position
                affected_component.is_active = False
                affected_component.removed_date = datetime.now(timezone.utc)
                action_taken = f"Merged {action.ticker} into existing {action.acquirer_ticker} position"
            else:
                # Replace ticker
